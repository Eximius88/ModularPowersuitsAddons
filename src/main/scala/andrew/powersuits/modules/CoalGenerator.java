...
public class CoalGenerator extends PowerModuleBase implements IPlayerTickModule, IToggleableModule {
    ...
    public CoalGenerator(List<IModularItem> validItems) {
        ...
        addInstallCost(new ItemStack(Block.furnaceIdle));
            ...
            for (int i = 0; i < inv.getSizeInventory(); i++) {
                ...
                if (stack != null && stack.itemID == Item.coal.itemID) {
                    ...
                    for (int i2 = 0; i2 < loopTimes; i2++) {
                        AddonUtils.setCoalLevel(item, AddonUtils.getCoalLevel(item) + 1);
                        player.inventory.decrStackSize(i, 1);
                        if (stack.stackSize == 0) {
                            player.inventory.setInventorySlotContents(i, null);
                        }
                    }
                    if (ModuleManager.computeModularProperty(item, MAX_COAL_STORAGE) - AddonUtils.getCoalLevel(item) < 1) {
    ...
    public String getLocalizedName() {
        return StatCollector.translateToLocal("module.coalGenerator.name");
    }
